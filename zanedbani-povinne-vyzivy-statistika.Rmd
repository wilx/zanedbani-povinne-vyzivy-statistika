...
zkrácením doby neplacení výživného, po které se stane neplacní trestným
činnem [@Kopecky2016]. ČSSD to bere z jiné strany a skrze ministryni práce a
...
obzvláště v souvsilosti se zkracováním lhůty, po které se z neplacení stane
trestný čin, jaký vliv má na počty neplatičů výživného
nezaměstnanost. Nezdálo se mi, že by zkrácení této lhůty významě přispělo ke
...
porovna zjištěné tresné činy zanedbání povinné výživy s HDP podle Českého
Statistikého Úřadu (ČSÚ), nezaměstnaností podle ČSÚ, nezaměstnaností podle
...
nejlépe korelující se ukázala být hodnota ročního průměru počtu evidovaných
uchazečů o práci (MPSV) v daném roce.
...
```{r, echo=FALSE}
tblColNames <- c("rok", "zjištěno", "stíháno osob", "z toho ženy", "paragraf", "nezaměstnanost v % (MPSV)", "počet evidovaných uchazečů (průměr, MPSV)", "nově hlášených 1.-12. (MPSV)")
...
Korelace nutně neznamená příčinu-důsledek, ale korlační koeficient `fmtNumM(Corr)` mezi zjištěnými trestnými činy zanedbání povinné výživy a počty registrovaných nezaměstnaných je poměrně vysoký. Tento vztah jistě stojí za hlubší zkoumání. Pravděpodobnost, že nezaměstnanost způsobuje alespoň část výskytů neplacení výživného se mi zdá velmi vysoká.
