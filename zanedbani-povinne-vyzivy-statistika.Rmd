...
ggplot(DF,aes(pocEvid, zjistenych)) +
    stat_smooth(formula = y ~ x, method="glm", level=0.99, fullrange = TRUE) +
    geom_point() +
...
zjistenychGLM.CI <- confint(zjistenychGLM, level = 0.99)
```
...
\(CI_{0,99}=(`r fmtNumM(zjistenychGLM.CI["(Intercept)",]["0.5 %"])`; `r fmtNumM(zjistenychGLM.CI["(Intercept)",]["99.5 %"])`)\).
Sklon linárního modelu má interval spolehlivosti
\(CI_{0,99}=(`r fmtNumM(zjistenychGLM.CI["DF$pocEvid",]["0.5 %"])`; `r fmtNumM(zjistenychGLM.CI["DF$pocEvid",]["99.5 %"])`)\).
