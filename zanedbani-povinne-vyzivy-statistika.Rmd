...
```{r, echo=FALSE, fig.keep="high", fig.cap="Korelace zjištěných trestných činů zanedbání povinné výživy a počtu evidovaných nezaměstnaných (MPSV ČR)", dpi=200}
pdf.options(encoding = 'CP1250')
...
BCorrTest <- bayes.cor.test(DF$zjistenych, DF$pocEvid, conf.level = 0.99, method = "pearson")
...
Pearsonův korelační koeficient mezi zjištěnými trestnými činy zanedbání povinné výživy a počty registrovaných nezaměstnaných v letech `r min(DF$rok)` až `r max(DF$rok)` je \(r(`r fmtNumM(CorrTest$parameter)`)=`r fmtNumM(Corr)`\), \({CI}_{`r attr(CorrTest$conf.int, "conf.level")`}=(`r fmtNumM(CorrTest$conf.int[1])`; `r fmtNumM(CorrTest$conf.int[2])`)\), \(p=`r fmtNumM(CorrTest$p.value)`\), \(r^2=`r fmtNumM(Corr*Corr)`\). Odhad korelačního koeficientu Bayesovou metodou \(\widetilde{\rho}=`r fmtNumM(BCorrTestSummary["rho",]["median"])`\), \({HDI}_{`r BCorrTestSummary["rho",]["HDI%"]/100.0`}=(`r fmtNumM(BCorrTestSummary["rho",]["HDIlo"])`; `r fmtNumM(BCorrTestSummary["rho",]["HDIup"])`)\).
